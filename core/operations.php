...
if(isset($_POST['operation_name'])) {
  ...
  if($settings->getSlack()) {
    sendSlackNotification('DOGFT Operations Calendar', 'pings', 'New Fleet Op Posted: '.$_POST['operation_name'].' - '.$_POST['operation_date'].' @ '.$_POST['operation_time'].' FCed by '.$_POST['operation_fc'].'. Form up in '.$_POST['operation_rally'].' on '.$_POST['operation_comms'], 'aura', $settings->getSlackWebhook());
